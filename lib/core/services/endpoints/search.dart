...
import 'package:inmusic/core/services/service.dart';
  ...
  Future<List<Map>> search(
    ...
    for (var e in contents) {
      ...
      ctx['contents'].forEach((a) {
        ...
        if (type == 'top resul') {
          type = a['flexColumns']?[1]['musicResponsiveListItemFlexColumnRenderer']['text']['runs'][0]['text']
              .toLowerCase();
        }
        if (!['artist', 'playlist', 'song', 'video', 'station'].contains(type)) {
          type = 'album';
        }
        // Map result = {
        ...
        //       [0]['text'];
        // }
        List data = a['flexColumns'][1]['musicResponsiveListItemFlexColumnRenderer']['text']['runs'];
        ...
        if (type != 'artist') {
          res['title'] = a['flexColumns'][0]['musicResponsiveListItemFlexColumnRenderer']['text']['runs'][0]['text'];
        }
        if (type == 'artist') {
          res['artist'] = a['flexColumns'][0]['musicResponsiveListItemFlexColumnRenderer']['text']['runs'][0]['text'];
          res['subscribers'] = data.last['text'];
        } else if (type == 'album') {
          res['type'] = a['flexColumns'][1]['musicResponsiveListItemFlexColumnRenderer']['text']['runs'][0]['text'];
        } else if (type == 'playlist') {
        ...
        res['thumbnails'] = a['thumbnail']['musicThumbnailRenderer']['thumbnail']['thumbnails'];
        ...
        for (Map run in data) {
          ...
          if (['artist', 'album', 'playlist'].contains(type)) {
            ...
            if (res['browseId'] == null && a['navigationEndpoint']['browseEndpoint']['browseId'] != null) {
              res['browseId'] = a['navigationEndpoint']['browseEndpoint']['browseId'];
            }
          }
        }
        ...
        if ((res['type'] == 'album' || res['type'] == 'song' || res['type'] == 'video') && res['artists'].isEmpty) {
        } else {
